...
def gen_bspline(data_image):
    ...
    """
    grid_size = 6  # Does not work, modify the following lines manually if necessary -- Bin
    image = sitk.GetImageFromArray(data_image)
    # bspline = sitk.BSplineTransformInitializer(image, [grid_size/2, grid_size, grid_size])
    # the next line works but the line above does not work, weird thing...
    bspline = sitk.BSplineTransformInitializer(image, [3, 6, 6])
...
def augment_subdataset_split(args, subfolder, id_list, data_suffixes, aug_name, augment=True, multiplier=1):
    ...
    """
    input_shape = np.asanyarray(read_nii_header(os.path.join(args.data_dir,subfolder), id_list[0], data_suffixes[0]).dataobj).shape
    ...
    for m in tqdm(range(len(id_list) * multiplier)):
        ...
        for j, s in enumerate(data_suffixes):
            ...
            image = np.asanyarray(image_handle.dataobj).astype(np.float32)
        ...
        np.savez_compressed(os.path.join(args.aug_data_dir,aug_name + str(m) + config.suffix_data), sample.astype(np.float32), allow_pickle=False)
        np.savez_compressed(os.path.join(args.aug_data_dir,aug_name + str(m) + config.suffix_data_cropped), sample_cropped.astype(np.float32), allow_pickle=False)

        label_handle = read_nii_header(os.path.join(args.data_dir,subfolder), f, config.suffix_seg)
        label_data = np.asanyarray(label_handle.dataobj).astype(np.float32)
        ...
        np.savez_compressed(os.path.join(args.aug_data_dir,aug_name + str(m) + config.suffix_label), label_data.astype(np.float32), allow_pickle=False)
        np.savez_compressed(os.path.join(args.aug_data_dir,aug_name + str(m) + config.suffix_label_cropped), label_data_cropped.astype(np.float32), allow_pickle=False)
